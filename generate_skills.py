#!/usr/bin/env python3
"""Generate SKILL.md files for the DispatcherAgents Freight Swarm (Brokerage).
...
# ROUTES/AGENTS single source: freight_data.py (fork-drift fix 2026-07-18)
from freight_data import ROUTES, AGENTS
...
def build(a):
    ...
    s += f"**Swarm:** DispatcherAgents Freight Swarm (Brokerage)\n"
    s += f"**Type:** {a['type']}\n"
...
- Verify signatures on human-authority intents (`ratecon.authority`,
...
- Enforce loop protection: a per-(`client_context_id`, intent) rate threshold.
  Exceeding it (e.g., 02↔03 re-vet ping-pong on a borderline carrier record) suspends the
...
- Operate the queues (queue name = intent string, exactly):
 ...
 - `escalation.hos_safety` / `escalation.double_broker` - human notification per
    configured urgency.
