---
name: 02-load-pipeline
description: "Load pipeline. Use when loads need lifecycle orchestration from tender through paper-complete delivery - vetting, assignment, tracking, and exceptions; the price is never pipeline arithmetic."
---

# Agent 02 - Load Pipeline Agent
...
Owns each load's swarm-side lifecycle: from captured tender through
...
| OUT | → 06 / 07 / 11 / 12 / 13 | Load change mid-cycle - blast radius named | `load.change.notice` |
| IN | ← 03 | Carrier standing changed mid-cycle (insurance, authority, safety) | `carrier.status.change` |
